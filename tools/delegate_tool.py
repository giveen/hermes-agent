...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "Two modes: (1) single task via 'goal' + optional context/toolsets, "
        f"or (2) parallel batch via 'tasks' array (up to {max_children} concurrent). "
        "Each subagent gets its own conversation, terminal, and tools. "
        "Only the final summary is returned to your context.\n\n"
        "Both modes run in background — delegate_task returns immediately. "
        "Results re-enter the conversation as new messages. "
        "Do NOT wait or poll.\n\n"
        f"Roles: {nesting_clause}. "
        "Leaf subagents cannot delegate/clarify/use memory/send_message/execute_code. "
        "Pass 'model', 'provider', or 'base_url' to route subagents to a different backend. "
        "Subagents inherit your config when unset. "
        "Results are always an array, one entry per task."
    )
...
def _build_tasks_param_description() -> str:
    ...
    try:
        ...
        max_children = _DEFAULT_MAX_CONCURRENT_CHILDREN
    return (
        f"Batch mode: tasks to run in parallel (max {max_children}). "
        "Each gets its own subagent. When set, top-level goal/context ignored."
    )
