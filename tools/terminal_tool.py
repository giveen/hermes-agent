...
TERMINAL_TOOL_DESCRIPTION = """Execute shell commands. Environment (cwd, exported vars) persists between calls.

Foreground (default): command returns when done. Set timeout high for long tasks.
Background: set background=true (+ notify_on_complete=true for bounded tasks). Servers/watchers skip notify.
Use process(action='poll') for progress, process(action='wait') to block.
PTY mode (pty=true) for interactive CLI tools (Codex, Claude Code, Python REPL).

Use read_file/search_files/patch/write_file instead of shell tools for file operations.
"""
...
TERMINAL_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "background": {
                "type": "boolean",
                "description": "Run in background. Pair with notify_on_complete=true for bounded tasks (tests, builds). Servers/watchers do not need notify — they never exit. For short commands, prefer foreground with generous timeout.",
                "default": False
            },
            "timeout": {
                "type": "integer",
            ...
            "notify_on_complete": {
                "type": "boolean",
                "description": "Notify once on process exit (background mode). Use for bounded tasks (tests, builds, deploys). Mutually exclusive with watch_patterns.",
                "default": False
            },
            "watch_patterns": {
                "type": "array",
                "items": {"type": "string"},
                "description": "Strings to watch in background output. Rate-limited: 1 notification per 15s; auto-promotes to notify_on_complete after 3 dropped windows. Use ONLY for rare one-shot signals on long-lived servers/daemons (e.g. startup complete). NOT for end-of-run markers — use notify_on_complete instead."
            }
        },
        "required": ["command"]
    }
