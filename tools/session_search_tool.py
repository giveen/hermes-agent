...
SESSION_SEARCH_SCHEMA = {
    "name": "session_search",
    "description": (
        "Search past Hermes sessions via FTS5 over the SQLite message store. "
        "No LLM calls — results are actual DB messages.\n\n"
        "Four shapes:\n"
        "  1) DISCOVERY: session_search(query=\"keywords\", limit=N)\n"
        "     FTS5 search, returns top sessions with match excerpts + bookends.\n"
        "  2) SCROLL: session_search(session_id=X, around_message_id=Y, window=N)\n"
        "     Window of ±N messages around an anchor, no search.\n"
        "  3) READ: session_search(session_id=X)\n"
        "     Full session by id (first 20 + last 10 when large).\n"
        "  4) BROWSE: session_search()\n"
        "     Recent sessions chronologically, no query needed.\n\n"
        "FTS5 syntax: AND is default. Use OR for broader recall, "
        "quoted phrases for exact match, prefix wildcards (deploy*).\n\n"
        "Use this for questions about conversation history. "
        "When the user gives a direct source (URL, file path, live system), "
        "inspect that source first — this tool only shows past conversation state."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "role_filter": {
                "type": "string",
                "description": (
                    "Comma-separated roles. Default 'user,assistant'. "
                    "Include 'tool' for tool output."
                ),
            },
            "profile": {
                "type": "string",
                "description": (
                    "Profile for cross-profile read (optional). "
                    "Omit for current profile."
                ),
            },
        },
        "required": [],
