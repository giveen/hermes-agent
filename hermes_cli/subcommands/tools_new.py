...
def build_tools_parser(subparsers, *, cmd_tools: Callable) -> None:
    ...
    tools_postsetup_p = tools_sub.add_parser(
        ...
        description=(
            ...
            "needs extra dependencies (browser Chromium, Camofax, cua-driver,\n"
            "KittenTTS/Piper, ddgs, Langfuse, xAI). Stable,\n"
            "non-interactive target the dashboard spawns to drive backend\n"
            "setup. Keys: agent_browser, camofax, cua_driver, kittentts,\n"
    ...
    tools_postsetup_p.add_argument(
        "post_setup_key",
        metavar="KEY",
        help="Post-setup hook key (e.g. agent_browser, camofax, kittentts)",
    )
