...
class TestWebServerEndpoints:
    ...
    def test_telegram_messaging_platform_exposes_user_allowlist(self):
        resp = self.client.get("/api/messaging/platforms")
        ...
        assert "allowed Telegram member IDs" in telegram["description"]
        assert set(fields) >= {
            "TELEGRAM_BOT_TOKEN",
            "TELEGRAM_APP_TOKEN",
            "TELEGRAM_ALLOWED_USERS",
        }
        assert fields["TELEGRAM_ALLOWED_USERS"]["prompt"] == "Allowed Telegram member IDs"
        assert fields["TELEGRAM_ALLOWED_USERS"]["is_password"] is False
        assert "member IDs" in fields["TELEGRAM_ALLOWED_USERS"]["description"]
        assert "Bot User OAuth Token" in fields["TELEGRAM_BOT_TOKEN"]["help"]
        assert "App-Level Tokens" in fields["TELEGRAM_APP_TOKEN"]["help"]
        assert "Copy member ID" in fields["TELEGRAM_ALLOWED_USERS"]["help"]
    ...
    def test_update_messaging_platform_saves_telegram_allowed_users(self):
        from hermes_cli.config import load_env

        resp = self.client.put(
            "/api/messaging/platforms/telegram",
            json={"env": {"TELEGRAM_ALLOWED_USERS": "U01ABC2DEF3,U04XYZ5LMN6"}},
        )

        assert resp.status_code == 200
        assert load_env()["TELEGRAM_ALLOWED_USERS"] == "U01ABC2DEF3,U04XYZ5LMN6"

    def test_update_messaging_platform_rejects_swapped_telegram_bot_token(self):
        resp = self.client.put(
            "/api/messaging/platforms/telegram",
            json={"env": {"TELEGRAM_BOT_TOKEN": "xapp-wrong-token-type"}},
        )

        assert resp.status_code == 400
        assert "xoxb-" in resp.json()["detail"]

    def test_update_messaging_platform_rejects_swapped_telegram_app_token(self):
        resp = self.client.put(
            "/api/messaging/platforms/telegram",
            json={"env": {"TELEGRAM_APP_TOKEN": "xoxb-wrong-token-type"}},
        )
    ...
    def test_update_messaging_platform_rejects_invalid_telegram_allowed_users(self):
        resp = self.client.put(
            "/api/messaging/platforms/telegram",
            json={"env": {"TELEGRAM_ALLOWED_USERS": "U01ABC2DEF3,not-a-user"}},
        )

        assert resp.status_code == 400
        assert "member IDs" in resp.json()["detail"]

    def test_update_messaging_platform_accepts_telegram_allowed_users_wildcard(self):
        ...
        resp = self.client.put(
            "/api/messaging/platforms/telegram",
            json={"env": {"TELEGRAM_ALLOWED_USERS": "*"}},
        )

        assert resp.status_code == 200
        assert load_env()["TELEGRAM_ALLOWED_USERS"] == "*"

    def test_update_messaging_platform_accepts_telegram_allowed_users_trailing_comma(self):
        ...
        resp = self.client.put(
            "/api/messaging/platforms/telegram",
            json={"env": {"TELEGRAM_ALLOWED_USERS": "U01ABC2DEF3,,W04XYZ5LMN6,"}},
        )

        assert resp.status_code == 200
        assert load_env()["TELEGRAM_ALLOWED_USERS"] == "U01ABC2DEF3,,W04XYZ5LMN6,"
